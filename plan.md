...
- 状态：进行中
- 实际 PR：#14
- 当前范围：本地化 `defineOverridesPreferences` identity helper，保留应用侧偏好类型约束，不迁移偏好管理器、缓存或持久化逻辑
...
- 当前阶段：PR #14 正在本地化 `defineOverridesPreferences` identity helper
...
- 下一步：完成 PR #14 CI、review 和合并，再选择下一个依赖边界清晰的最小运行时单元
